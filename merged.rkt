...
;;;Purpose:draw concentric circles of c-num on the image with radius of width or height/5
...
(define draw-magic-circle3
  (lambda (num width height)
    (let* ([nums (split-num num)]
      ...
      (cond [(not (= 0 tnum))
      (for-each draw-pos-triangle (make-list tnum image) (map  (l-s * width) (map (r-s / 10) (iota tnum)))
                                                 (map  (l-s * height) (map (r-s / 10) (iota tnum))))])
      (cond [(not (= 0 snum))
      (for-each draw-neg-triangle (make-list snum image) (map  (l-s * width) (map (r-s / 10) (iota snum)))
  ...
  (lambda (num width height)
   (let* ([nums (split-num num)]
     ...
     (for-each draw-central-circles (make-list 5 image) (make-list 5 2)
                                    ...
                                    (map (r-s + (* height 0.03))inner-pentagon-row))
         (cond [(not (= 0 tnum))
           (for-each draw-central-circles (make-list 5 image) (make-list 5 tnum)
                                    (map (r-s - (* width 0.05)) outer-pentagon-col)
                                    (map (r-s - (* height 0.05)) outer-pentagon-row)
                                    (map (r-s + (* width 0.05)) outer-pentagon-col)
                                    (map (r-s + (* height 0.05)) outer-pentagon-row))])
      (cond [(not (= 0 snum))
       (draw-central-circles image snum (- c-col (* 0.1 width))
                                        (- c-row (* 0.1 height))
                                        (+ c-col (* 0.1 width))
                                        (+ c-row (* 0.1 height)))])
      (image-refresh-display! image)
